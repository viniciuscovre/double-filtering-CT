function [y_est] = BM3D_PA(z, profile, print_to_screen)
y = 1;
sigma = 1;
%sigma = 1 %variância do ruído no caso de anscombe é sempre approx. 1

z = noise_transform(z,'ansc');
M = max(z(:));
m = min(z(:));
z = ((z-m)/(M-m));
sigma = sigma / (M - m);
...
y_est = y_est * (M-m) + m; %voltou à escala original
clear M; clear m;
y_est = noise_transform(y_est,'ansc_inverse');
return;
